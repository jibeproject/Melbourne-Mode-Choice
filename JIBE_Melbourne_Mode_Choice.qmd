...
To ensure proper coding, `origplace1` will be recorded as work related (**for now, pending review with colleagues**)
```{r}
...
trips <- trips %>% 
    ...
    within({
        trips.purpose = case_when(
        trips.origin == "Place of Personal Business" | trips.destination == "B" ~ "business", ## is this right?
        trips.origin %in% c("NA", "Not Stated")          | 
            is.NA(trips.origin) ~ "unknown"            |
            trips.destination %in% c("NA", "Not Stated") | 
            is.NA(trips.destination) 
              ~ "unknown",
        trips.origin == "RRT" | trips.destination == "RRT" ~ "RRT", ## This case doesn't exist in our data 
        trips.origin == "Accommodation" ~ case_when(
            trips.destination == "Workplace" ~ "HBW", 
            trips.destination == "Education" ~ "HBE",
            trips.destination == "Shops" ~ "HBS",
            trips.destination %in% c("Natural Feature","Recreational Place","Social Place") ~ "HBR",
            trips.destination == "Other" ~ "HBO",
            trips.destpurp1 == "Accompany Someone" ~ "HBA",  ## Should we use purpose of 'Accompany Someone', even if Destination suggests something else? 
            ),
        trips.destination == "Accommodation" ~ "NA", # return trip
        trips.origin == "Workplace" | trips.destination == "Workplace" ~ "NHBW",
        TRUE ~ "NHBO")
    
        ## Further define purpose for return trips
        trips.full_purpose = case_when(
            trips.purpose == "NA" ~ case_when(
                trips.origin == "Workplace" ~ "HBW",
                trips.origin == "Place of Education" ~ "HBE",
                trips.origin == "Shops" ~ "HBS",
                trips.origin %in% c("Natural Feature","Recreational Place","Social Place") ~ "HBR",
                trips.origin == "Other" ~ "HBO",
                trips.destpurp1 == "Accompany Someone" ~ "HBA"),
            TRUE ~ trips.purpose
            )
})
```
