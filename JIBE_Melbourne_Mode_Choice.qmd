...
```{r}
#| label: select.H_VISTA_1220_Coord.csv
...
I am not sure off hand how best to classify this multi-trip paradigm, that is distinct from other single trip classifications, in code. 

*Update* The issue of multiple trips was addressed further in an email from Corin dated 2025-02-14, where he indidcated that no further action seemed necessary at this stage for round trips represented by multiple consecutive legs; it was sufficient for each leg to remain recorded by reference to the destination purpose of that particular leg. 


### Further trip cleaning prior to assignment, based on above review

To ensure proper coding, `origplace1`/`destplace1` will be recorded as work related where it is NA or Unknown and origin/destination purpose is 'Work Related' or 'Unknown'
...
Also, we will replace these respective NA or Unknown values as 'workplace' to ensure we have the appropriate fields in order to categorise as NHBW, later.
...
Save the trips table with purposes included (used as an input in `time_of_day/time_of_day.R`, `mode_share/modeShare.R` and `mean_trip_length/avgTripLength.R`) and persons table (used as an input in `mean_trip_length/avgTripLength.R`).

```{r}
#| output: true
#| label: save-outputs.MITO
saveRDS(trips, "./trips_with_purpose.rds")
saveRDS(survey$P, "./persons.rds")
```
```{r}
#| output: true
#| label: mode_share.select.SA1shapefile.then.calibrationRegions.csv
source('./mode_share/modeShare.R')
```
```{r}
#| output: true
#| label: mean_trip_length.select.SA1shapefile
